...
# Clean Sandboxed Gemini Interface Import
from core.gemini_flasher import render_gemini_flasher_interface
...
def _avg_daily_deployed(sub_df: "pd.DataFrame", today: "pd.Timestamp") -> float:
    """
    True average daily capital deployed across a set of trades.

    For every calendar day from first entry to today:
        deployed_on_day = sum(EntryPrice × Qty) for trades where
                          entry_date <= day <= exit_date
    Open positions use today as exit date.
    ExitDate column used when present; falls back to today if missing or NaT.
    Returns the mean of daily deployed values.
    """
    if sub_df.empty:
        return 0.0

    ep  = pd.to_numeric(sub_df.get("EntryPrice", pd.Series(dtype=float)),
                        errors="coerce")
    qty = pd.to_numeric(sub_df.get("Qty", pd.Series(dtype=float)),
                        errors="coerce")
    ...
    # open trades or missing exit → deployed through today
    ...
    # Peak daily deployed = max concurrent capital on any active day.
    # This represents the capital envelope for the strategy — what the
    # engine needed at maximum capacity — giving the most honest ROC.
...
def _engine_return_table(trades_df):
    ...
    for eng in engines:
        try:
            ...
            # --- denominator: average daily capital actually deployed --------
            # For each calendar day this engine was active, we sum the capital
            # tied up in live positions (EntryPrice × Qty). Average across all
            # days gives the true working-capital base for this engine.
...
# ----- Combined portfolio CAGR (avg daily deployed capital) ---------------
# Sum what was actually working each day across all engines combined.
_today_norm = pd.Timestamp.today().normalize()
total_alloc  = _avg_daily_deployed(trades, _today_norm) if not trades.empty else 0.0
...
st.caption(
    "CAGR = annualised return on peak daily deployed capital "
    "(EntryPrice × Qty, summed across all live positions each day, averaged "
    "over the full period). ROC is the raw period return on that base. "
    "Both update daily as the track record grows."
)
...
@st.cache_data(ttl=300, show_spinner=False)
def _calc_engine_health(trades_df):
    """
    Rolling health diagnostics per engine from closed trades.
    Returns one row per engine with: win rate, expectancy, losing streak,
    drawdown from peak, profit factor, risk-reward, and overall health score.
    """
    if trades_df.empty or "Engine" not in trades_df.columns:
        return pd.DataFrame()

    if "Status" not in trades_df.columns or "PnL" not in trades_df.columns:
        return pd.DataFrame()

    closed = trades_df[trades_df["Status"].astype(str).str.upper() == "CLOSED"].copy()
    if closed.empty:
        return pd.DataFrame()

    closed["_pnl"] = pd.to_numeric(closed["PnL"], errors="coerce").fillna(0)
    if "Date" in closed.columns:
        closed["_dt"] = pd.to_datetime(closed["Date"], errors="coerce")
        closed = closed.sort_values("_dt")

    ROLL = 20
    rows = []

    for eng in sorted(closed["Engine"].dropna().astype(str).unique()):
        ec = closed[closed["Engine"].astype(str) == eng].copy()
        if ec.empty:
            continue
        ...
        # ── 1. Rolling win rate (last 20 trades) ──────────────────
        wins     = int((recent > 0).sum())
        win_rate = wins / w * 100

        # ── 2. Expectancy per trade ───────────────────────────────
        avg_win  = float(recent[recent > 0].mean()) if (recent > 0).any() else 0.0
        avg_loss = float(abs(recent[recent < 0].mean())) if (recent < 0).any() else 0.0
        win_pct  = wins / w
        expectancy = win_pct * avg_win - (1.0 - win_pct) * avg_loss

        # ── 3. Current losing streak (most recent backwards) ─────
        streak = 0
        for v in pnl[::-1]:
            if v < 0:
                streak += 1
        ...
        # ── 4. Max drawdown as % of DEPLOYED CAPITAL ────────────
        # Denominator = avg position size × 3 (max concurrent per engine).
        # Drawdown from peak P&L divided by this capital base, NOT by
        # peak P&L — avoids the paradox of profitable engine showing
        # -24% drawdown from a small profit pullback.
        cum = np.cumsum(pnl)
        running_max = np.maximum.accumulate(cum)
        dd_abs = float((cum - running_max).min()) if len(cum) else 0.0
        _all_eng = trades_df[trades_df["Engine"].astype(str) == eng]
        _pos_sz = (
            pd.to_numeric(_all_eng.get("EntryPrice", pd.Series(dtype=float)),
                          errors="coerce")
            * pd.to_numeric(_all_eng.get("Qty", pd.Series(dtype=float)),
                            errors="coerce")
        ).dropna()
        _cap_base = (float(_pos_sz.mean() * 3)
                     if not _pos_sz.empty and float(_pos_sz.mean()) > 0
                     else 0.0)
        if _cap_base > 0:
            dd_pct = dd_abs / _cap_base * 100.0
        elif dd_abs < 0:
            dd_pct = -100.0
        else:
            dd_pct = 0.0

        # ── 5. Profit factor (rolling window) ────────────────────
        gw = float(recent[recent > 0].sum()) if (recent > 0).any() else 0.0
        gl = float(abs(recent[recent < 0].sum())) if (recent < 0).any() else 0.001
        pf = gw / gl

        # ── 6. Risk-reward ratio ─────────────────────────────────
        rr = avg_win / avg_loss if avg_loss > 0 else (99.9 if avg_win > 0 else 0.0)

        # ── 7. Composite health score (0-100) ────────────────────
        sc = 0
        # win rate (max 25)
        sc += 25 if win_rate >= 65 else 15 if win_rate >= 55 else 8 if win_rate >= 50 else 0
        # expectancy (max 25)
        sc += 25 if expectancy >= 200 else 18 if expectancy >= 100 else 10 if expectancy >= 50 else (5 if expectancy > 0 else 0)
        # streak (max 15)
        sc += 15 if streak <= 2 else 8 if streak <= 4 else 0
        # drawdown (max 20) — user's thresholds: -15% yellow, -21% red
        sc += 20 if dd_pct >= -10 else 12 if dd_pct >= -15 else 5 if dd_pct >= -21 else 0
        # profit factor (max 15)
        sc += 15 if pf >= 1.5 else 8 if pf >= 1.0 else 0

        status_label = "HEALTHY" if sc >= 70 else "WATCH" if sc >= 50 else "CRITICAL"
        # Hard override: drawdown breach triggers alert regardless of score
        if dd_pct < -21.0:
            status_label = "CRITICAL"
        elif dd_pct < -15.0 and status_label == "HEALTHY":
            status_label = "WATCH"

        rows.append({
            "Engine": eng, "Trades": n, "Win%(20)": round(win_rate, 1),
            "Expectancy": round(expectancy, 0), "Streak": streak,
            "Drawdown%": round(dd_pct, 1), "PF": round(pf, 2),
            "R:R": round(rr, 2), "Score": sc, "Status": status_label,
        })
...
with tab1:
    ...
    if signals.empty:
        ...
        # ── Live CMP — reuses existing 60s-cached _live_prices ──────────────────
        _sig_tickers = tuple(disp["Ticker"].dropna().astype(str).unique().tolist())
        _sig_px = _live_prices(_sig_tickers) if _sig_tickers else {}
        _ins = disp.columns.get_loc("Ticker") + 1 if "Ticker" in disp.columns else len(disp.columns)
        # _live_prices returns bare keys (no .NS/.BO) — strip suffix before mapping
        _bare_series = disp["Ticker"].astype(str).str.replace(r"\.NS$|\.BO$", "", regex=True)
        disp.insert(_ins, "CMP", _bare_series.map(_sig_px))

        # Ensure numeric types so column_config can right-align them
        for _nc in ["Entry", "SL", "Target", "Capital", "Qty", "Score", "CMP"]:
            if _nc in disp.columns:
                disp[_nc] = pd.to_numeric(disp[_nc], errors="coerce")

        # Column config: tight widths + right-align numbers
        _col_cfg = {}
        _widths = {
            "Date": 90, "Engine": 65, "Ticker": 125, "Action": 65,
            "CMP": 98, "Entry": 98, "SL": 98, "Target": 98,
            "Qty": 58, "Capital": 102, "Sector": 108, "Score": 68, "Status": 76,
        }
        for _c, _w in _widths.items():
            if _c not in disp.columns:
                continue
            if _c in ("Entry", "SL", "Target", "Capital", "CMP"):
                _col_cfg[_c] = st.column_config.NumberColumn(_c, format="₹%.2f", width=_w)
            elif _c == "Qty":
                _col_cfg[_c] = st.column_config.NumberColumn(_c, format="%d", width=_w)
            elif _c == "Score":
                _col_cfg[_c] = st.column_config.NumberColumn(_c, format="%.2f", width=_w)
            else:
                _col_cfg[_c] = st.column_config.TextColumn(_c, width=_w)

        st.dataframe(disp, use_container_width=True, hide_index=True, column_config=_col_cfg)
...
with tab4:
    ...
    st.caption("Rolling 20-trade diagnostics per engine · updates from trades log")

    health_df = _calc_engine_health(trades.copy())

    if health_df.empty:
        st.info("Need closed trades to calculate health metrics. Run engines and close some trades first.")
    else:
        # ── alert banners ──────────────────────────────────────────────
        _crit = health_df[health_df["Status"] == "CRITICAL"]
        _watch = health_df[health_df["Status"] == "WATCH"]
        if not _crit.empty:
            st.error(
                f"🚨  CRITICAL — "
                f"{', '.join(_crit['Engine'].tolist())} — "
                f"pause and review before next capital deployment"
            )
        if not _watch.empty:
            st.warning(
                f"⚠️  WATCH — "
                f"{', '.join(_watch['Engine'].tolist())} — "
                f"performance weakening, monitor next 10 trades closely"
            )

        # ── per-engine health cards ────────────────────────────────────
        _hcards = st.columns(max(len(health_df), 1))
        for _hi, _hr in health_df.iterrows():
            _eng    = _hr["Engine"]
            _score  = _hr["Score"]
            _hstat  = _hr["Status"]

            if _hstat == "HEALTHY":
                _hacc, _hdot = CLR_BULL, "🟢"
            elif _hstat == "WATCH":
                _hacc, _hdot = CLR_WARN, "🟡"
            else:
                _hacc, _hdot = CLR_BEAR, "🔴"

            # metric colour helpers
            _wr_c  = CLR_BULL if _hr["Win%(20)"] >= 65 else CLR_WARN if _hr["Win%(20)"] >= 55 else CLR_BEAR
            _ex_c  = CLR_BULL if _hr["Expectancy"] >= 200 else CLR_WARN if _hr["Expectancy"] >= 50 else CLR_BEAR
            _sk_c  = CLR_BULL if _hr["Streak"] <= 2 else CLR_WARN if _hr["Streak"] <= 4 else CLR_BEAR
            _dd_c  = CLR_BULL if _hr["Drawdown%"] >= -10 else CLR_WARN if _hr["Drawdown%"] >= -15 else (CLR_WARN if _hr["Drawdown%"] >= -21 else CLR_BEAR)
            _pf_c   = CLR_BULL if _hr["PF"] >= 1.5 else CLR_WARN if _hr["PF"] >= 1.0 else CLR_BEAR
            _rr_c   = CLR_BULL if _hr["R:R"] >= 1.5 else CLR_WARN if _hr["R:R"] >= 1.0 else CLR_BEAR
            _pf_str = "\u221e" if _hr["PF"] >= 99.0 else f'{_hr["PF"]:.2f}'
            _rr_str = "\u221e" if _hr["R:R"] >= 99.0 else f'{_hr["R:R"]:.2f}'

            with _hcards[_hi]:
                st.markdown(
                    f'<div class="qc-engine-card" style="border-top:3px solid {_hacc};">'

                    # header — engine + score badge
                    f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">'
                    f'<span style="color:{TXT_PRIMARY};font-size:16px;font-weight:800;">{_eng}</span>'
                    f'<span style="background:{_hacc}22;color:{_hacc};padding:3px 10px;border-radius:12px;'
                    f'font-size:14px;font-weight:700;">{_hdot} {_score}</span>'
                    f'</div>'

                    # status label
                    f'<div style="color:{_hacc};font-size:14px;font-weight:700;letter-spacing:0.8px;'
                    f'margin-bottom:10px;">{_hstat}</div>'

                    # metrics rows
                    f'<div style="font-size:14px;line-height:2.2;">'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">Win Rate (20)</span>'
                    f'<b style="color:{_wr_c};">{_hr["Win%(20)"]:.0f}%</b></div>'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">Expectancy</span>'
                    f'<b style="color:{_ex_c};">Rs.{_hr["Expectancy"]:.0f}</b></div>'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">Lose Streak</span>'
                    f'<b style="color:{_sk_c};">{_hr["Streak"]}</b></div>'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">Drawdown</span>'
                    f'<b style="color:{_dd_c};">{_hr["Drawdown%"]:.1f}%</b></div>'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">Profit Factor</span>'
                    f'<b style="color:{_pf_c};">{_pf_str}</b></div>'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">R:R Ratio</span>'
                    f'<b style="color:{_rr_c};">{_rr_str}</b></div>'

                    f'<div style="display:flex;justify-content:space-between;">'
                    f'<span style="color:{TXT_SECOND};">Total Trades</span>'
                    f'<b style="color:{TXT_PRIMARY};">{_hr["Trades"]}</b></div>'
        ...
        st.caption(
            "Thresholds — "
            "Win Rate: 🟢 ≥65% 🟡 ≥55% 🔴 <55%  ·  "
            "Drawdown: 🟢 ≥-10% 🟡 ≥-15% 🔴 <-21% (pause & review)  ·  "
            "Streak: 🟢 ≤2 🟡 ≤4 🔴 ≥5  ·  "
            "Profit Factor: 🟢 ≥1.5 🟡 ≥1.0 🔴 <1.0"
        )

    # ── CAPITAL READINESS ──────────────────────────────────────────────
    st.markdown('<div class="qc-section-title">Capital Readiness</div>', unsafe_allow_html=True)
    st.caption("Engine classification based on current health, consistency, and track record maturity")
    ...
    _cr_tiers = [
        {
            ...
            "color": CLR_BULL,
            "icon": "🟩",
            "desc": "Proven edge · consistent returns · primary capital engines",
        },
        {
            "tier": "SCALE",
            "engines": ["E6"],
            "color": CLR_INFO,
            "icon": "🟦",
            "desc": "Strong early results · ready for gradual capital increase",
        },
        {
            "tier": "INCUBATION",
            "engines": ["E5", "G1"],
            "color": CLR_CYAN,
            "icon": "🔷",
            "desc": "Developing track record · maintain current capital · observe",
        },
        {
            "tier": "WATCH",
            "engines": ["E3"],
            "color": CLR_WARN,
            "icon": "🟧",
            "desc": "Weakening metrics · no new capital · review strategy logic",
        },
        {
            "tier": "PAUSE",
            "engines": ["E4"],
            "color": CLR_BEAR,
            "icon": "🟥",
            "desc": "Edge not confirmed · capital deployment paused · rework needed",
        },
    ]

    _cr_cols = st.columns(len(_cr_tiers))
    for _ci, _ct in enumerate(_cr_tiers):
        _tc   = _ct["color"]
        _eng_list = "  ·  ".join(_ct["engines"])
        with _cr_cols[_ci]:
            st.markdown(
                f'<div class="qc-engine-card" style="border-top:3px solid {_tc};min-height:140px;">'

                # tier badge
                f'<div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;">'
                f'<span style="font-size:16px;">{_ct["icon"]}</span>'
                f'<span style="color:{_tc};font-size:15px;font-weight:800;'
                f'letter-spacing:1.2px;">{_ct["tier"]}</span>'
                f'</div>'

                # engine names
                f'<div style="color:{TXT_PRIMARY};font-size:18px;font-weight:700;'
                f'margin-bottom:10px;">{_eng_list}</div>'

                # description
                f'<div style="color:{TXT_SECOND};font-size:14px;line-height:1.6;">'
                f'{_ct["desc"]}</div>'
    ...
    # ── ENGINE RUN STATUS ──────────────────────────────────────────────
    st.markdown('<div class="qc-section-title">Engine Run Status</div>', unsafe_allow_html=True)
    ...
    if status.empty:
        ...
        if not latest_per_engine.empty:
            _rcards = st.columns(max(len(latest_per_engine), 1))
            for i, row in enumerate(latest_per_engine.itertuples(index=False)):
                ...
                with _rcards[i]:
                    st.markdown(
                        ...
                        f'<span style="color:{TXT_PRIMARY};font-size:13px;font-weight:700;letter-spacing:1.2px;">{eng}</span>'
                        f'<span style="color:{TXT_SECOND};font-size:12px;">{ts_str}</span>'
                        f'</div>'
                        f'<div style="color:{acc};font-size:15px;font-weight:700;margin-top:6px;letter-spacing:0.4px;">{eng_st}</div>'
                        f'<div style="color:{TXT_SECOND};font-size:13px;margin-top:4px;">{detail}</div>'
                        f'</div>',
                        unsafe_allow_html=True,
                    )
                ...
                for i, (reason, cnt) in enumerate(reason_counts.items()):
                    with rcols[i]:
                        st.markdown(
                            ...
                            f'<div style="color:{TXT_MUTED};font-size:12px;font-weight:700;letter-spacing:1px;text-transform:uppercase;">{str(reason)[:24]}</div>'
    ...
    if not news_items:
        ...
        for it in filtered:
            ...
            st.markdown(
                ...
                f'<span style="background:{color}22;color:{color};border:1px solid {color}44;'
                f'border-radius:5px;padding:2px 9px;font-size:11px;font-weight:700;letter-spacing:0.4px;">{bare}</span>'
                f'<span style="background:{BG_HOVER};color:{TXT_SECOND};border-radius:5px;'
                f'padding:2px 8px;font-size:11px;font-weight:600;">{badge}</span>'
                f'<span style="color:{TXT_MUTED};font-size:12px;">{provider}</span>'
                f'</div>'
                f'<span style="color:{TXT_DIM};font-size:12px;white-space:nowrap;">{ago}</span>'
                f'</div>'
                ...
                f'<div style="margin-top:6px;color:{TXT_DIM};font-size:12px;">{pub}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
        ...
        if load_ok:
            ...
            for i, a in enumerate(action_order):
                ...
                with action_cols[i]:
                    st.markdown(
                        ...
                        f'<div style="color:{TXT_MUTED};font-size:11px;letter-spacing:1px;font-weight:700;">{a}</div>'
            ...
            if not live.empty:
                ...
                with bw1:
                    st.markdown(
                        ...
                        f'<span style="color:{CLR_BULL};font-size:12px;font-weight:700;letter-spacing:1px;">BEST</span>'
                ...
                with bw2:
                    st.markdown(
                        ...
                        f'<span style="color:{CLR_BEAR};font-size:12px;font-weight:700;letter-spacing:1px;">WORST</span>'
            ...
            if not booked.empty:
                ...
                st.markdown(
                    ...
                    f'<span style="color:{TXT_MUTED};font-size:12px;letter-spacing:1px;">TOTAL BOOKED P&L &nbsp;</span>'
...
AI_PAPER_TRADES_FILE = Path(__file__).resolve().parent / "data" / "ai_paper_trades.csv"

def _pt_cols():
    return [
        "AddedDate", "AddedTS", "Source", "Ticker", "Score",
        "Entry", "SL", "Target", "Qty", "Risk",
        "Status", "ExitPrice", "ExitDate",
    ]
...
def _save_paper_trade(r, source: str = "AI Scanner") -> bool:
    """Append one manual paper trade from a scanner SignalResult."""
    df = _load_paper_trades()
    if not df.empty:
        if not df[(df["Ticker"] == r.ticker) & (df["Status"] == "OPEN")].empty:
            return False   # already tracking this ticker
    ...
    new_row = {
        ...
        "Entry":     round(float(r.suggested_entry),  2),
        "SL":        round(float(r.suggested_stop),   2),
        "Target":    round(float(r.suggested_target), 2),
        "Qty":       int(r.suggested_qty),
        "Risk":      round(float(r.max_risk_inr), 0),
        "Status":    "OPEN",
        "ExitPrice": np.nan,
        "ExitDate":  np.nan,
    }
    df = pd.concat([df, pd.DataFrame([new_row])], ignore_index=True)
    AI_PAPER_TRADES_FILE.parent.mkdir(parents=True, exist_ok=True)
    df.to_csv(AI_PAPER_TRADES_FILE, index=False)
    return True

def _exit_paper_trade(ticker: str, exit_price=None) -> bool:
    """Mark the latest OPEN paper trade for ticker as EXITED."""
    if not AI_PAPER_TRADES_FILE.exists():
        return False
    df = pd.read_csv(AI_PAPER_TRADES_FILE)
...
def _enrich_paper_trades(df: pd.DataFrame) -> pd.DataFrame:
    """Add live CMP, Return %, P&L ₹, Days held, and Outcome."""
    if df.empty:
        return df
    df = df.copy()
...
# ═══════════════════════════════════════════════════════════════════
AI_SHADOW_BOOK_FILE = Path(__file__).resolve().parent / "data" / "ai_shadow_book.csv"

def _shadow_book_columns():
    return [
        "SnapshotDate", "SnapshotTS", "Rank", "Ticker", "Company", "Sector",
        "Score", "Conviction", "Entry ₹", "SL ₹", "Target ₹", "Qty",
        "Risk ₹", "1M %", "RSI", "vs Nifty", "Catalyst", "Key Signal",
    ]

def _empty_shadow_book() -> pd.DataFrame:
    return pd.DataFrame(columns=_shadow_book_columns())

def _load_shadow_book() -> pd.DataFrame:
    path = AI_SHADOW_BOOK_FILE
    if not path.exists():
        return _empty_shadow_book()
    ...
    for col in ["SnapshotDate", "SnapshotTS"]:
        if col in df.columns:
            df[col] = pd.to_datetime(df[col], errors="coerce")

    for col in _shadow_book_columns():
        if col not in df.columns:
            df[col] = np.nan

    return df[_shadow_book_columns()]

def _build_shadow_snapshot(top_buys, scanned_at) -> pd.DataFrame:
    if not top_buys:
        return _empty_shadow_book()

    snap_ts = pd.Timestamp(scanned_at)
    snap_date = snap_ts.normalize()
    rows = []

    for rank, r in enumerate(top_buys[:3], start=1):
        bulls = getattr(r, "bull_signals", None) or []
        rows.append({
            "SnapshotDate": snap_date,
            "SnapshotTS": snap_ts,
            "Rank": rank,
            "Ticker": str(getattr(r, "ticker", "")).upper().strip(),
            "Company": str(getattr(r, "company_name", ""))[:40],
            "Sector": str(getattr(r, "sector", "—"))[:24],
            "Score": float(getattr(r, "composite_score", 0) or 0),
            "Conviction": int(getattr(r, "conviction", 0) or 0),
            "Entry ₹": float(getattr(r, "suggested_entry", np.nan) or np.nan),
            "SL ₹": float(getattr(r, "suggested_stop", np.nan) or np.nan),
            "Target ₹": float(getattr(r, "suggested_target", np.nan) or np.nan),
            "Qty": float(getattr(r, "suggested_qty", np.nan) or np.nan),
            "Risk ₹": float(getattr(r, "max_risk_inr", np.nan) or np.nan),
            "1M %": float(getattr(r, "ret_1m", 0) or 0),
            "RSI": float(getattr(r, "rsi", 0) or 0),
            "vs Nifty": float(getattr(r, "rs_vs_nifty_60d", 0) or 0),
            "Catalyst": str(getattr(r, "next_event_label", "—") or "—"),
            "Key Signal": " · ".join(bulls[:3]) if bulls else "—",
        })

    df = pd.DataFrame(rows, columns=_shadow_book_columns())
    return df

def _save_shadow_snapshot(snapshot_df: pd.DataFrame) -> bool:
    if snapshot_df.empty:
        return False

    path = AI_SHADOW_BOOK_FILE
    path.parent.mkdir(parents=True, exist_ok=True)
    existing = _load_shadow_book()

    snap_date = pd.to_datetime(snapshot_df["SnapshotDate"].iloc[0], errors="coerce")
    if pd.notna(snap_date) and not existing.empty and "SnapshotDate" in existing.columns:
        ex_dates = pd.to_datetime(existing["SnapshotDate"], errors="coerce").dt.normalize()
        if (ex_dates == snap_date.normalize()).any():
            return False

    combined = pd.concat([existing, snapshot_df], ignore_index=True)
    combined = combined[_shadow_book_columns()]
    combined.to_csv(path, index=False)
    return True

def _latest_shadow_view(book_df: pd.DataFrame) -> pd.DataFrame:
    if book_df.empty:
        return _empty_shadow_book()

    df = book_df.copy()
    df["SnapshotDate"] = pd.to_datetime(df["SnapshotDate"], errors="coerce")
    df["SnapshotTS"] = pd.to_datetime(df["SnapshotTS"], errors="coerce")

    if df["SnapshotDate"].notna().any():
        today = pd.Timestamp.today().normalize()
        today_mask = df["SnapshotDate"].dt.normalize() == today
        if today_mask.any():
            return (
                df.loc[today_mask]
        ...
        latest_dt = df["SnapshotDate"].dt.normalize().max()
        if pd.notna(latest_dt):
            return (
                df.loc[df["SnapshotDate"].dt.normalize() == latest_dt]
                .sort_values(["Rank", "Ticker"], ascending=[True, True])
                .reset_index(drop=True)
            )

    return (
        df.sort_values(["SnapshotTS", "Rank"], ascending=[False, True])
        .head(3)
        .reset_index(drop=True)
    )

def _live_shadow_metrics(snapshot_df: pd.DataFrame) -> pd.DataFrame:
    if snapshot_df.empty:
        return _empty_shadow_book()
    ...
    live_px = _live_prices(tickers) if tickers else {}

    def _px(t: str):
        key = str(t).upper().replace(".NS", "").replace(".BO", "")
        return live_px.get(key)

    df["CMP ₹"] = df["Ticker"].apply(_px)
    ...
    df["Paper Return %"] = ((df["CMP ₹"] - df["Entry ₹"]) / df["Entry ₹"]) * 100.0
    df["Paper P&L ₹"] = (df["CMP ₹"] - df["Entry ₹"]) * df["Qty"]
    df["Days"] = (pd.Timestamp.today().normalize() - df["SnapshotDate"].dt.normalize()).dt.days
    ...
    def _outcome(row):
        ...
        if pd.isna(cmpv):
            return "NO DATA"
        if pd.notna(tgt) and cmpv >= tgt:
            return "TP"
        if pd.notna(sl) and cmpv <= sl:
            return "SL"
        return "OPEN"
...
def _shadow_summary(df: pd.DataFrame) -> dict:
    if df.empty:
        return {
            "total": 0, "open": 0, "tp": 0, "sl": 0,
            "hit_rate": np.nan, "avg_ret": np.nan, "avg_pnl": np.nan,
        }

    total = len(df)
    tp = int((df["Outcome"] == "TP").sum()) if "Outcome" in df.columns else 0
    sl = int((df["Outcome"] == "SL").sum()) if "Outcome" in df.columns else 0
    open_n = int((df["Outcome"] == "OPEN").sum()) if "Outcome" in df.columns else 0
    closed = tp + sl
    hit_rate = (tp / closed * 100.0) if closed else np.nan
    avg_ret = float(pd.to_numeric(df.get("Paper Return %", pd.Series(dtype=float)), errors="coerce").mean()) if "Paper Return %" in df.columns else np.nan
    avg_pnl = float(pd.to_numeric(df.get("Paper P&L ₹", pd.Series(dtype=float)), errors="coerce").mean()) if "Paper P&L ₹" in df.columns else np.nan
    ...
    if not scan or not scan.get("all_results"):
        ...
        if exits:
            ...
            for r in exits:
                ...
                st.markdown(
                    ...
                    f'<div style="display:flex;justify-content:space-between;align-items:center;">'
                    f'<div>'
                    f'<span style="color:{CLR_BEAR};font-weight:700;letter-spacing:0.5px;font-size:13px;">EXIT</span>'
                    f'&nbsp;&nbsp;<span style="color:{TXT_PRIMARY};font-weight:700;font-size:16px;">{r.ticker}</span>'
                    f'&nbsp;<span style="color:{TXT_MUTED};font-size:13px;">{r.company_name[:36]}</span>'
                    f'</div>'

                    f'<div style="display:flex;gap:18px;align-items:center;">'
                    f'<span style="color:{TXT_MUTED};font-size:12px;">Score '
                    f'<b style="color:{CLR_BEAR};">{r.composite_score:.0f}</b></span>'
                    f'<span style="color:{TXT_MUTED};font-size:12px;">Current P&L '
                    f'<b style="color:{pnl_color};">{pnl_str}</b></span>'
                    f'<span style="color:{TXT_MUTED};font-size:12px;">CMP '
                    f'<b style="color:{TXT_PRIMARY};">₹{r.current_price:,.2f}</b></span>'
                    f'</div>'

                    f'</div>'

                    f'<div style="color:{TXT_DIM};font-size:13px;margin-top:6px;">{bears_str}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )
        ...
        if strong_buys:
            ...
            for i, r in enumerate(top3):
                with cols[i]:
                    ...
                    cat_html = (
                        ...
                        f'border-radius:5px;padding:1px 8px;font-size:11px;font-weight:600;margin-left:6px;">'
                    ...
                    st.markdown(
                        ...
                        f'{r.ticker}</div>'
                        f'<div style="color:{TXT_MUTED};font-size:15px;margin-top:2px;">{r.company_name[:30]}</div>'
                        f'<div style="color:{TXT_DIM};font-size:13px;margin-top:1px;">{r.sector[:30]}</div>'
                        f'</div>'
                        f'<div style="color:#fbbf24;font-size:15px;letter-spacing:1px;">{stars}</div>'
                        f'</div>'
                        ...
                        f'border-radius:5px;padding:2px 10px;font-size:11px;font-weight:700;letter-spacing:0.4px;">'
                        f'STRONG BUY</span>'
                        f'<span style="background:{BG_HOVER};color:{TXT_SECOND};border-radius:5px;'
                        f'padding:2px 8px;font-size:11px;font-weight:600;">Score {r.composite_score:.0f}</span>'
                        f'<span style="background:{BG_HOVER};color:{TXT_SECOND};border-radius:5px;'
                        f'padding:2px 8px;font-size:11px;">⚡ {r.groups_fired}/5</span>'
                        f'{cat_html}'
                        f'</div>'
                        ...
                        f'<div style="display:grid;grid-template-columns:1fr 1fr;gap:5px;font-size:12px;">'
                        ...
                        f'<div style="color:{TXT_MUTED};font-size:12px;line-height:1.5;'
                    ...
                    if st.button(
                        "📋 Add Paper Trade",
                        key=f"ai_pt_{r.ticker}_{i}",
                        use_container_width=True,
                    ):
                        _added = _save_paper_trade(r, source="Top Opportunities")
                        if _added:
                            st.session_state[f"ai_pt_ok_{r.ticker}"] = True
                        else:
                            st.session_state[f"ai_pt_dup_{r.ticker}"] = True
                        st.rerun()
                    if st.session_state.pop(f"ai_pt_ok_{r.ticker}", False):
                        st.success(f"✓ {r.ticker} added — see tracker below")
                    if st.session_state.pop(f"ai_pt_dup_{r.ticker}", False):
                        st.warning(f"{r.ticker} already in open paper trades")
            ...
            if rows:
                ...
                # ── Quick-add from full list ───────────────────────
                with st.expander("📋 Add any Strong Buy to Paper Trades"):
                    _qa_tickers = [r.ticker for r in strong_buys]
                    _qa_map     = {r.ticker: r for r in strong_buys}
                    _qa_sel     = st.selectbox(
                        "Select ticker", _qa_tickers, key="ai_qa_sel"
                    )
                    if st.button("Add to Paper Trades", key="ai_qa_btn", type="primary"):
                        _ok = _save_paper_trade(_qa_map[_qa_sel], source="All Strong Buys")
                        if _ok:
                            st.success(f"✓ {_qa_sel} added — see tracker below")
                        else:
                            st.warning(f"{_qa_sel} already has an open paper trade")
                        st.rerun()
        ...
        # ──────────────────────────────────────────────────────
        # SHADOW TRADE BOOK — frozen daily top-3 accountability
        # ──────────────────────────────────────────────────────
        shadow_snapshot = _build_shadow_snapshot(strong_buys, scanned_at)
        if not shadow_snapshot.empty:
            _save_shadow_snapshot(shadow_snapshot)

        shadow_book = _load_shadow_book()
        shadow_view = _latest_shadow_view(shadow_book)
        shadow_live = _live_shadow_metrics(shadow_view)

        if not shadow_live.empty:
            st.markdown(
                '<div class="qc-section-title" style="margin-top:20px;">'
                'Shadow Trade Book</div>',
                unsafe_allow_html=True,
            )
            st.caption(
                "Frozen at the first successful scan of the day; live CMP updates on every refresh."
            )

            shadow_sum = _shadow_summary(shadow_live)
            sr1, sr2, sr3, sr4, sr5, sr6 = st.columns(6)
            with sr1:
                st.metric("Logged", f"{shadow_sum['total']}")
            with sr2:
                st.metric("Open", f"{shadow_sum['open']}")
            with sr3:
                st.metric("TP", f"{shadow_sum['tp']}")
            with sr4:
                st.metric("SL", f"{shadow_sum['sl']}")
            with sr5:
                hr = shadow_sum["hit_rate"]
                st.metric("Hit Rate", "—" if pd.isna(hr) else f"{hr:.1f}%")
            with sr6:
                ar = shadow_sum["avg_ret"]
                st.metric("Avg Return", "—" if pd.isna(ar) else f"{ar:+.2f}%")

            st.caption(
                "Accountability view for the day's top 3: snapshot is fixed at first scan, "
                "then monitored live against the original entry, stop-loss, and target."
            )

            display_cols = [
                "SnapshotDate", "Rank", "Ticker", "Company", "Sector", "Score",
                "Conviction", "Entry ₹", "CMP ₹", "Paper Return %", "Paper P&L ₹",
                "SL ₹", "Target ₹", "Outcome", "Days", "Risk ₹", "1M %", "RSI",
                "vs Nifty", "Key Signal",
            ]
            disp = shadow_live[[c for c in display_cols if c in shadow_live.columns]].copy()
            ...
            if "Date" in disp.columns:
                fmt["Date"] = lambda x: pd.Timestamp(x).strftime("%d-%b") if pd.notna(x) else "—"
            for c in ["Entry ₹", "CMP ₹", "SL ₹", "Target ₹", "Risk ₹", "P&L ₹"]:
                if c in disp.columns:
                    fmt[c] = "₹{:,.2f}" if c in ["Entry ₹", "CMP ₹", "SL ₹", "Target ₹", "Risk ₹"] else "₹{:,.0f}"
            for c in ["Return %", "1M %", "vs Nifty"]:
                if c in disp.columns:
                    fmt[c] = "{:+.2f}%" if c == "Return %" else "{:+.1f}%"
            ...
            def _paper_num(val):
                try:
                    v = float(str(val).replace("%", "").replace("₹", "").replace(",", "").replace("+", ""))
                    if v > 0:
                        return f"color:{CLR_BULL};font-weight:600"
                    if v < 0:
                        return f"color:{CLR_BEAR};font-weight:600"
                except Exception:
                    pass
                return ""

            colored_cols = [c for c in ["Return %", "P&L ₹", "Score", "1M %", "vs Nifty"] if c in disp.columns]
            try:
                styled_shadow = disp.style.apply(_paper_bg, axis=1).map(_paper_num, subset=colored_cols).format(fmt, na_rep="—")
            except AttributeError:
                styled_shadow = disp.style.apply(_paper_bg, axis=1).applymap(_paper_num, subset=colored_cols).format(fmt, na_rep="—")

            st.dataframe(styled_shadow, use_container_width=True, hide_index=True)

        # ── AI PAPER TRADE TRACKER ─────────────────────────────────
        st.markdown(
            '<div class="qc-section-title" style="margin-top:24px;">'
            'AI Paper Trade Tracker</div>',
            unsafe_allow_html=True,
        )
        st.caption(
            "Manual paper trades from the scanner. "
            "Live CMP refreshes every 3 min. "
        ...
        if _pt_raw.empty:
            st.info(
                "No paper trades yet. Click **📋 Add Paper Trade** on any "
                "Strong Buy card above to start tracking."
            )
        else:
            _pt = _enrich_paper_trades(_pt_raw)

            # ── Summary KPIs ──────────────────────────────────────
            _open   = int((_pt["Outcome"] == "OPEN").sum())
            _tp     = int(_pt["Outcome"].str.startswith("TP").sum())
            _sl     = int(_pt["Outcome"].str.startswith("SL").sum())
            _exited = int((_pt["Outcome"] == "EXITED").sum())
            _closed = _tp + _sl + _exited
            _hr     = _tp / _closed * 100 if _closed > 0 else np.nan
            _pnl    = _pt["P&L ₹"].sum()

            _pc = st.columns(6)
            with _pc[0]: st.metric("Total",     len(_pt))
            with _pc[1]: st.metric("Open",      _open)
            with _pc[2]: st.metric("TP ✓",      _tp)
            with _pc[3]: st.metric("SL ✗",      _sl)
            with _pc[4]: st.metric("Hit Rate",  "—" if pd.isna(_hr)  else f"{_hr:.1f}%")
            with _pc[5]: st.metric("Total P&L", "—" if pd.isna(_pnl) else f"₹{_pnl:,.0f}")

            # ── Tracker table ─────────────────────────────────────
            _show = [
                "AddedDate", "Ticker", "Score", "Entry", "CMP",
                "SL", "Target", "Qty", "Return %", "P&L ₹", "Days", "Outcome",
            ]
            _disp = _pt[[c for c in _show if c in _pt.columns]].copy()
            ...
            if "Qty"      in _disp: _ptfmt["Qty"]      = "{:.0f}"
            if "Return %" in _disp: _ptfmt["Return %"] = "{:+.2f}%"
            if "P&L ₹"   in _disp: _ptfmt["P&L ₹"]   = "₹{:,.0f}"
            if "Days"     in _disp: _ptfmt["Days"]     = "{:.0f}"

            def _pt_bg(row):
                out = str(row.get("Outcome", ""))
                bg  = ("#10b98118" if "TP"   in out else
                       "#ef444418" if "SL"   in out else
                       "#94a3b812" if "EXIT" in out else
                       "#3b82f610")
            ...
            def _pt_num(v):
                try:
                    ...
                    if x > 0: return f"color:{CLR_BULL};font-weight:700"
                    if x < 0: return f"color:{CLR_BEAR};font-weight:700"
                except Exception:
                    pass
                return ""

            _rc = [c for c in ["Return %","P&L ₹"] if c in _disp.columns]
            try:
            ...
            # ── Exit a trade ──────────────────────────────────────
            _open_tkrs = _pt.loc[_pt["Outcome"] == "OPEN", "Ticker"].unique().tolist()
            if _open_tkrs:
                with st.expander("🔴 Exit / Close a paper trade"):
                    _ex1, _ex2, _ex3 = st.columns([3, 2, 1])
                    with _ex1:
                        _ex_sel = st.selectbox("Select ticker to exit", _open_tkrs, key="ai_pt_exit_sel")
                    with _ex2:
                        _ex_px  = st.number_input("Exit price (0 = live CMP)", min_value=0.0, value=0.0, step=0.5, key="ai_pt_exit_px")
                    with _ex3:
                        st.write("")
                        if st.button("Exit", key="ai_pt_exit_btn", type="primary"):
                            if _exit_paper_trade(_ex_sel, _ex_px if _ex_px > 0 else None):
                                st.success(f"✓ {_ex_sel} marked as EXITED")
                                st.rerun()
